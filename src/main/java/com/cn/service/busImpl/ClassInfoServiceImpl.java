...
public class ClassInfoServiceImpl implements ClassInfoService {
    ...
    public JSONObject insertStudent(String jsonstr) {
        ...
        JSONObject jo = new JSONObject(jsonstr);
        String classId = jo.getString("classId");//班级ID
        String studentId = jo.getString("studentId");//用户ID
        String createName = jo.has("create") ? jo.getString("create") : "";//创建人-微信标识
        if (Utils.isEmpty(studentId) || Utils.isEmpty(classId) ) {
            result.put(MsgAndCode.RSP_CODE, MsgAndCode.CODE_001);
            result.put(MsgAndCode.RSP_DESC, MsgAndCode.CODE_001_MSG);
            return result;
        } else {
            try {
                ...
                if (classInfo != null) {
                    Conjunction cn = Restrictions.conjunction();
                    cn.add(Restrictions.eq("classStudent", studentId));
                    cn.add(Restrictions.eq("classId", classId));
                    List<ClassStudentInfo> csi = baseEntityDao.listByCriteria(ClassStudentInfo.class, cn, false);
                    if(csi.size()==0)
                    {
                        //根据班级ID 插入学生信息
                        ...
                        result.put(MsgAndCode.RSP_CODE, MsgAndCode.CODE_017);
                        result.put(MsgAndCode.RSP_DESC, MsgAndCode.CODE_017_MSG);
                    }
    ...
    public JSONObject getClassInfoList(String jsonstr) {
        ...
        if (Utils.isEmpty(curragePage) || Utils.isEmpty(pageSize)) {
            ...
            try {
                ...
                if (!Utils.isEmpty(studentId)) {
                    sql.append(" AND C.CLASS_ID IN (SELECT CS.CLASS_ID  FROM CLASS_STUDENT_INFO CS WHERE CS.CLASS_ID =C.CLASS_ID ");
                    sql.append("  AND CS.CLASS_STUDENT ='" + studentId + "')");
                }
